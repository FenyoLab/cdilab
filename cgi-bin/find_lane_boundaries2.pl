...
{
	if ($hdri_file)
	{# ImageMagick pixel enumeration: 238,271,65535,gray
		...
		{#the first line of the file, with the # pixels in x, y and max value for rgb data
			...
			$y_max=$2;
			$color_max=$3;
		} # 0,0: (112.793%,112.793%,112.793%)  #FFFFFFFFFFFF  gray(112.793%)
		elsif($line=~/^([0-9]+),([0-9]+):\s*\(\s*([0-9\.]+)%,\s*([0-9\.]+)%,\s*([0-9\.]+)%\)\s*#[A-Z0-9]+\s*gray\(([0-9\.]+)%\)/)
...
for($lane_i = 0; $lane_i < $num_found_lanes; $lane_i++)
{
	...
	while ($line=<IN>)
	{
		...
		elsif(($hdri_file && $line=~/^([0-9]+),([0-9]+):\s*\(\s*([0-9\.]+)%,\s*([0-9\.]+)%,\s*([0-9\.]+)%\)\s*#[A-Z0-9]+\s*gray\(([0-9\.]+)%\)/) ||
			(!$hdri_file && $line=~/^([0-9]+),([0-9]+):\s\(\s*([0-9]+)%?,\s*([0-9]+)%?,\s*([0-9]+)%?[,\)]/))
		{
			my $x=$1;
			my $y=$2;
			...
			if($x >= $start_x && $x <= $end_x) 
			{
				...
				#{ print OUT qq!$x_,$y:(65535,    0,    0)  #FFFF00000000  red\n!; }
				{
					if ($hdri_file)
					{
						#110,1258: (66500%,66500%,66500%)  #FFFFFFFFFFFF  gray(66500%)
						print OUT qq!$x_,$y: (0%,0%,0%)  #FFFFFFFFFFFF  gray(0%)\n!;
					}
					elsif ($type eq 'graya')
					{
						#print OUT qq!$x_,$y:(255,255,255,255)  #FFFFFF  srgb(255,255,255,1)\n!;
	...
	if (!$hdri_file) #rotate doesn't work right for hdri
	{
		#rotate the lane:
